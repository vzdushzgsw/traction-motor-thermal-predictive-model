...
Instead of using randomized dummy data, this machine learning algorithm is trained exclusively on synthetic, thermodynamics-based data generated by my connected repository: 
[EV Traction Motor Digital Twin Simulator](https://github.com/vzdushzgsw/ev_traction_motor_thermal_digital_twin).

**How it works:** It reads the `synthetic_motor_data.csv` generated by the Digital Twin, learns the exact physical boundaries where the stator insulation fails, and provides an interactive command-line interface to simulate a live Vehicle Control Unit (VCU) clamping the current (Turtle Mode) when dangerous thresholds are reached.
